...
Json parse_json_parameter_value(std::string_view value) {
    Json parsed = Json::parse(value, nullptr, false);
    if (!parsed.is_discarded()) { return parsed; }

    // Qwen occasionally renders scalar arguments using Python spellings
    // (True/False/None) even though the tool contract is JSON-shaped. These
    // values are unambiguous scalar literals; normalize only the exact
    // spellings and keep malformed objects/arrays rejected below.
...
bool parse_parameter(std::string_view inner, std::size_t& pos, Json& args,
    ...
    if (contract == nullptr) {
        ...
        if (contract->encoding == ToolArgumentTypeContracts::Encoding::String) {
            ...
            Json parsed = parse_json_parameter_value(value);
