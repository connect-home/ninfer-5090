...
int test_declared_type_mismatches_are_forwarded_without_coercion() {
    ...
    const std::string python_boolean =
        "<tool_call>\n<function=configure>\n<parameter=python_boolean>\nTrue\n</parameter>\n"
        "</function>\n</tool_call>";
    const auto accepted = fi::parse_qwen_tool_call_output(python_boolean, 64, contracts);
    failures += check(accepted.is_tool_call_response && accepted.tool_calls.size() == 1,
                      "Python-style boolean for a declared parameter did not parse");
    if (accepted.is_tool_call_response && accepted.tool_calls.size() == 1) {
        const Json normalized = Json::parse(accepted.tool_calls.at(0).arguments_json);
        failures += check(normalized.at("python_boolean").is_boolean() &&
                              normalized.at("python_boolean") == true,
                          "Python-style boolean was not normalized to JSON boolean");
    }
    return failures;
}
